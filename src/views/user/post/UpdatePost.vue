...
export default {
 ...
 computed:{
  ...mapState('post', ['updatepost'])
 },
 methods: {
    ...mapActions('auth', ['checkAuthUser']),
  setContent(){
   this.data.title = this.updatepost.postcontent.title
   this.data.content = this.updatepost.postcontent.content
   this.data.post_excerpt = this.updatepost.postcontent.post_excerpt
   this.data.id = this.updatepost.id
  },
  ...
  async updatePost(){
   ...
   const res = await this.$store.dispatch('post/updatePost', this.data)
