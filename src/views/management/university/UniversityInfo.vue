<template>
 <div>
  <div class="container pe-0 pe-sm-0 pe-md-2 pe-lg-4 pe-xl-4">
    ...
    <div class="col-12 col-sm-12 col-md-12 col-lg-5 col-xl-6">
     <div class="row justify-content-center m-0 mb-4">
      <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-10">
       <div
        class="card mt-2 sys-primary text-white br-20 ps-4 p-3"
        :class="
         msg == 'Good Morning'
          ? 'bg-morning'
          : '' || msg == 'Good Afternoon'
        ...
        <blockquote>
         <div class="d-flex justify-content-end">
          <a
           ...
           @click.prevent="
            mission = lnumission;
            $bvModal.show('updateMissionModal');
           "
         ...
         <span
          ><small>{{ lnumission.lnu_mission }}</small></span
         >
        </blockquote>
       </div>
      </slide>
        ...
        <blockquote>
         <div class="d-flex justify-content-end">
          <a
           ...
           @click.prevent="
            vision = lnuvision;
            $bvModal.show('updateVisionModal');
           "
           ><i class="bi bi-pencil-square"></i
          ></a>
         </div>
         ...
         <span
          ><small>{{ lnuvision.lnu_vision }}</small></span
         >
        </blockquote>
       </div>
      </slide>
        ...
        <blockquote>
         <div class="d-flex justify-content-end">
          <a
           ...
           @click.prevent="
            core_value = cv;
            $bvModal.show('updateCVModal');
           "
           ><i class="bi bi-pencil-square"></i
          ></a>
         </div>
         <span class="fw-bold">{{ cv.core_value.toUpperCase() }}</span
         ><br /><br />
         <span class=""
          ><small>{{ cv.description }}</small></span
         >
        </blockquote>
       </div>
      </slide>
     ...
     <div class="card p-5">
      ...
      <div class="table-responsive mt-4">
       <b-skeleton-table
        ...
        v-if="(initialLoading && officials.data == 0) || isOfficialTableLoading"
       ></b-skeleton-table>
        ...
        <tbody>
         <tr v-for="(official, i) in officials.data" :key="i">
          ...
          <td>
           <div class="d-flex">
            <button
             v-on:click.prevent="setUpdateAccount(official)"
             v-b-tooltip.hover
             title="Update"
            ...
            <button
             :disabled="isLoading && current_id == official.id"
             v-on:click.prevent="
              data.id = official.id;
             ...
             v-b-tooltip.hover
             title="Delete"
             class="btn btn-sm btn-danger rounded-pill btn-approve"
            >
             <i v-if="current_id != official.id" class="bi bi-trash"></i>
             <div
              v-if="isLoading && current_id == official.id"
   ...
   <div class="row  mt-5 mb-3 m-0 ps-lg-1 ps-xl-1 ps-md-4 pe-0 pe-md-4 pe-lg-2 pe-xl-5 mb-5">
    <div class="col-12 col-md-12 col-lg-6 col-xl-6">
     <h5>OTHER INFO</h5>
     <hr />
     <div class="d-flex justify-content-between">
      <h6 class="text-center fw-bold text-uppercase">History</h6>
      <a href="" class="text-decoration-none me-2" v-on:click.prevent="$bvModal.show('viewHistoryModal'); history = info[0].lnu_history">View</a>
     </div>
     <div class="d-flex justify-content-between mt-2">
      <h6 class="text-center fw-bold">QUALITY POLICY</h6>
      <a href="" class="text-decoration-none me-2"  v-on:click.prevent="$bvModal.show('viewQualityPolicyModal'); qualitypolicy = info[0].lnu_qualitypolicy">View</a>
     </div>
    </div>
   </div>
  </div>
  ...
  <!-- VIEW HISTORY MODAL --->
  <b-modal id="viewHistoryModal" centered title="LNU History">
   <p>History</p>
   <textarea v-model="history" class="form-control"></textarea>
   <template #modal-footer="{cancel}">
    <b-button variant="primary" @click="cancel()" :disabled="isLoading"> Close </b-button>
    <b-button variant="success" v-on:click.prevent="updateHistory" :disabled="isLoading">
     Update
    </b-button>
   </template>
  </b-modal>

  <!-- VIEW QUALITY POLICY MODAL --->
  <b-modal id="viewQualityPolicyModal" centered title="LNU Quality Policy">
   <p>Quality Policy</p>
   <textarea v-model="qualitypolicy" class="form-control"></textarea>
   <template #modal-footer="{cancel}">
    <b-button variant="primary" @click="cancel()" :disabled="isLoading"> Close </b-button>
    <b-button variant="success" v-on:click.prevent="updateQualityPolicy" :disabled="isLoading">
     Update
    </b-button>
   </template>
  </b-modal>
  ...
  <b-modal id="updateCVModal" centered title="Update Core Value">
   ...
   <div class="form-floating mb-2 mt-4 ">
    <input
     v-model="core_value.description"
     type="text"
     class="form-control"
     id="updateCVDesc"
     placeholder=""
...
<script>
 ...
 export default {
  ...
  methods: {
   ...
   async updateQualityPolicy(){
     this.isLoading = true
      const data = {
       id: this.info[0].id,
       qualitypolicy: this.qualitypolicy
     }
     const res = await this.$store.dispatch('universityinfo/updateQualityPolicy', data)
     if (res.status == 200) {
      await this.getUniInfo();
      this.$toast.success('Quality Policy updated successfully!');
      this.$bvModal.hide('viewQualityPolicyModal');
     }
     this.isLoading = false;
   },
   async updateHistory(){
     ...
     const res = await this.$store.dispatch('universityinfo/updateHistory', data)
     if (res.status == 200) {
      await this.getUniInfo();
      this.$toast.success('History updated successfully!');
      this.$bvModal.hide('viewHistoryModal');
     }
     this.isLoading = false;
   },
   async updateMission() {
