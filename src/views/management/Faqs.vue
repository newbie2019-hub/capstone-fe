...
export default {
  ...
  async mounted(){
    ...
    await this.$store.dispatch('faq/getFaq', 1);
    this.initialLoading = false
  },
  methods: {
    async saveFAQ(){
      this.isLoading = true
      const res = await this.$store.dispatch('faq/saveFaq', this.faq)
      if(res.status == 200 ){
    ...
    async updateFAQ(){
      this.isLoading = true
      const res = await this.$store.dispatch('faq/updateFaq', this.update)
      if(res.status == 200 ){
    ...
    async destroyFAQ(){
      this.isLoading = true
      const res = await this.$store.dispatch('faq/deleteFaq', this.deleteFaq)
      if(res.status == 200){
    ...
    async searchFaqs(page){
       ...
       await this.$store.dispatch('faq/searchFaqs', {page: page, data: data})
       this.isSearching = false
    },
    async getFaqs(page){
       await this.$store.dispatch('faq/getFaq', page);
    },
    faqsSearch(page = 1){
      if(this.search_faq == ''){
        this.getFaqs(page)
